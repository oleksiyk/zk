...
var NativeZk = require('zookeeper/build/zookeeper.node').ZooKeeper;
...
// Watch event types
exports.ZOO_CREATED_EVENT     = 1
exports.ZOO_DELETED_EVENT     = 2
exports.ZOO_CHANGED_EVENT     = 3
exports.ZOO_CHILD_EVENT       = 4
exports.ZOO_SESSION_EVENT     = -1
exports.ZOO_NOTWATCHING_EVENT = -2
...
Zookeeper.prototype.connect = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        self.zk.init({
            connect: self.options.hostname + ':' + self.options.port,
            timeout: self.options.timeout,
            debug_level: self.ZOO_LOG_LEVEL_WARN,
...
Zookeeper.prototype.get = function(path, watch) {
    ...
    return new Promise(function(resolve, reject) {
        if(!watch){
            self.zk.a_get(path, null, function(rc, error, stat, data) {
                if(rc !== 0){
            ...
            self.zk.aw_get(path, function(type, state, path) {
                resolver.resolve({
...
Zookeeper.prototype.exists = function(path, watch) {
    ...
    return new Promise(function(resolve, reject) {
        if (!watch) {
            self.zk.a_exists(path, null, function(rc, error, stat) {
                if (rc !== 0) {
                    return reject(new ZkError(rc, error))
                }
                resolve({
            ...
            var resolver = Promise.defer()
            self.zk.aw_exists(path, function(type, state, path) {
                resolver.resolve({
                    type: _zkWatchEventTypes[type],
                    state: _zkStates[state],
                    path: path
...
Zookeeper.prototype.getChildren = function(path, watch) {
    ...
    return new Promise(function(resolve, reject) {
        if (!watch) {
            self.zk.a_get_children(path, null, function(rc, error, children) {
                if (rc !== 0) {
                    return reject(new ZkError(rc, error))
                }
                resolve({
            ...
            var resolver = Promise.defer()
            self.zk.aw_get_children(path, function(type, state, path) {
                resolver.resolve({
                    type: _zkWatchEventTypes[type],
                    state: _zkStates[state],
                    path: path
...
Zookeeper.prototype.getChildren2 = function(path, watch) {
    ...
    return new Promise(function(resolve, reject) {
        if (!watch) {
            self.zk.a_get_children2(path, null, function(rc, error, children, stat) {
                if (rc !== 0) {
                    return reject(new ZkError(rc, error))
                }
                resolve({
            ...
            var resolver = Promise.defer()
            self.zk.aw_get_children2(path, function(type, state, path) {
                resolver.resolve({
                    type: _zkWatchEventTypes[type],
                    state: _zkStates[state],
                    path: path

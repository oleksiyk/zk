...
var Lock = function(zk, name, options) {
    this.zk = zk
    this.name = name
    this.options = options || {}
    this.options.returnValue = this.options.returnValue || false
...
Lock.prototype.lock = function(f) {
    ...
    function grabLock () {
        ...
        return self.zk.getChildren('/zk-locks-' + self.name).then(function(reply) {
            ...
            if(ind === 0){ // we got the lock
                if(self.options.returnValue){
                    return self.zk.get('/zk-locks-' + self.name).get('data')
                }
                return
            }
    ...
    if(self.path){
        throw new Error(self.name + ': lock() already called on ' + self.path)
    }
    ...
    return self.lockNode.then(function() {
        ...
        .then(f)
        .then(function(val) {
            if(self.options.returnValue && (typeof val === 'string' || Buffer.isBuffer(val))){
                return self.zk.set('/zk-locks-' + self.name, val, -1).return(val)
            }
            return val
        })
        .finally(function() {
